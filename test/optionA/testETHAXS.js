...
const AXSAddress = addresses.TOKENS.AXS
...
const unlockedAddress2 = "0xE93381fB4c4F14bDa253907b18faD305D799241a"//addresses.ADDRESSES.unlockedUser2
...
describe("OA - ETHAXS", () => {
    const setup = async () => {
        ...
        const AXS = new ethers.Contract(AXSAddress, IERC20_ABI, deployer)
        ...
        const lpToken = new ethers.Contract(addresses.TOKENS.ETHAXSLP, IERC20_ABI, deployer)
        ...
        await AXS.connect(unlockedUser).approve(vault.address, ethers.utils.parseUnits("1000000000", 18))
        ...
        await AXS.connect(unlockedUser).approve(SushiRouter.address, ethers.utils.parseUnits("1000000000", 18))
        ...
        await AXS.connect(unlockedUser2).approve(vault.address, ethers.utils.parseUnits("1000000000", 18))
        ...
        await AXS.connect(unlockedUser2).approve(SushiRouter.address, ethers.utils.parseUnits("1000000000", 18))
        ...
        return { vault, USDT, AXS, DAI, WETH, lpToken, unlockedUser, unlockedUser2, adminSigner, deployer, SushiRouter }
    }

    beforeEach(async () => {
        await deployments.fixture(["oa_mainnet_deploy_pool_eth-axs"])
    })


    it("Should deploy correctly", async () => {
        const { vault, AXS, USDT, DAI, unlockedUser, unlockedUser2, adminSigner, deployer } = await setup()
    ...
    it("should work - normal flow", async () => {
        const { vault, AXS, USDT, DAI, WETH, lpToken, unlockedUser, unlockedUser2, adminSigner, deployer, SushiRouter } = await setup()
        await SushiRouter.connect(unlockedUser).swapExactTokensForTokens(ethers.utils.parseUnits("10000", 18), 0, [AXS.address, WETH.address], unlockedAddress, 32490605417);
        await SushiRouter.connect(unlockedUser).addLiquidity(AXS.address, WETH.address, ethers.utils.parseUnits("10000", 18), ethers.utils.parseUnits("3", 18), 0, 0, unlockedAddress, 32490605417);

        await SushiRouter.connect(unlockedUser2).swapExactTokensForTokens(ethers.utils.parseUnits("10000", 18), 0, [AXS.address, WETH.address], unlockedAddress2, 32490605417);
        await SushiRouter.connect(unlockedUser2).addLiquidity(AXS.address, WETH.address, ethers.utils.parseUnits("10000", 18), ethers.utils.parseUnits("3", 18), 0, 0, unlockedAddress2, 32490605417);
    ...
    it("Should yield correctly", async () => {
        const { vault, strategy, AXS, USDT, WETH, lpToken, unlockedUser, unlockedUser2, adminSigner, deployer, SushiRouter } = await setup()
        await SushiRouter.connect(unlockedUser).swapExactTokensForTokens(ethers.utils.parseUnits("10000", 18), 0, [AXS.address, WETH.address], unlockedAddress, 32490605417);
        await SushiRouter.connect(unlockedUser).addLiquidity(AXS.address, WETH.address, ethers.utils.parseUnits("10000", 18), ethers.utils.parseUnits("3", 18), 0, 0, unlockedAddress, 32490605417);
