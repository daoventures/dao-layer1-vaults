...
describe("DAO Earn", () => {
    ...
    beforeEach(async () => {
        await deployments.fixture(["earn_mainnet_deploy_vault_ankreth"])
    })

    it("Should work", async () => {
        const [deployer, client, strategist, biconomy, treasury, community] = await ethers.getSigners()
        const curveZap = await ethers.getContract("CurveMetaPoolFacZap")
        ...
        await lpTokenContract.transfer(client.address, ethers.utils.parseEther("5"))
        ...
        await lpTokenContract.transfer(sample.address, ethers.utils.parseEther("5"))
