...
const unlockedAddr = "0x807baf9e22f4e1e7a5fcf4b5721ba54666d71421"
...
describe("DAO Earn", () => {
    ...
    beforeEach(async () => {
        await deployments.fixture(["earn_mainnet_deploy_vault_reth"])
    })

    it("Should work", async () => {
        const [deployer, client, strategist, biconomy, treasury, community] = await ethers.getSigners()
        const curveZap = await ethers.getContract("CurvePlainPoolETHZap")
        ...
        await lpTokenContract.transfer(client.address, ethers.utils.parseEther("2"))
        ...
        await earnVault.connect(client).deposit(ethers.utils.parseEther("2"), false)
        ...
        await expect(earnVault.connect(client).deposit(ethers.utils.parseEther("2"), false)).to.be.revertedWith("Pausable: paused")
        // console.log(ethers.utils.formatEther(await cvStake.balanceOf(earnStrategy.address)))
        // console.log(ethers.utils.formatEther(await lpTokenContract.balanceOf(earnVault.address)))
