...
const { ethers, deployments, network, tenderly } = require('hardhat') 
const { mainnet: addresses } = require('../../addresses/uniswapV3') 
...
describe("UNI - SLP_ETH", async () => {
    ...
    it("Should transfer fee correctly", async () => {
        ...
        let two_Percent0 = ethers.utils.parseEther("2").mul(2).div(100)
        let four_Percent0 = ethers.utils.parseEther("4").mul(2).div(100)
        ...
        // console.log(strategistBalanceBefore0.toString(), strategistBalanceAfter0.toString());

        expect(strategistBalanceAfter0.sub(strategistBalanceBefore0)).to.be.gte(two_Percent0.div(2));
        expect(communityWalletBalanceAfter0.sub(communityWalletBalanceBefore0)).to.be.gte(four_Percent0.div(2));
        expect(treasuryBalanceAfter0.sub(treasuryBalanceBefore0)).to.be.gte(four_Percent0.div(2));
    })
