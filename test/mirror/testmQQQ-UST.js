...
const unlockedAddress = "0x2cee14757750ead3dec7f1c6818c467bb73f878d"//"0xc247722Ac42B2f9BA752886502c3D3dD39BDb2Da"//addresses.ADDRESSES.unlockedUser
...
describe("Mirror - mQQQ-UST", () => {
    const setup = async () => {
        const [deployer, user1, user2, topup] = await ethers.getSigners()

        const lpToken = new ethers.Contract(addresses.LPTOKENS.mQQQ_UST, IERC20_ABI, deployer)
        ...
        await lpToken.connect(unlockedUser).transfer(user1.address, ethers.utils.parseUnits("0.01", "18"))
        await lpToken.connect(unlockedUser2).transfer(user2.address, ethers.utils.parseUnits("0.006", "18"))
    ...
    beforeEach(async () => {
        await deployments.fixture(["mirror_mainnet_deploy_pool_mqqq_ust"])
    })
