...
const unlockedAddress = "0x5071af77f17050477d21350e6dbeac13b5e7928f"//"0xc247722Ac42B2f9BA752886502c3D3dD39BDb2Da"//addresses.ADDRESSES.unlockedUser
const unlockedAddress2 = "0xe9f6ab900a911c75cabd605a32cef24b889322b8" //"0xF39d30Fa570db7940e5b3A3e42694665A1449E4B"//addresses.ADDRESSES.unlockedUser2


describe("Mirror - mGOOGL-UST", () => {
    const setup = async () => {
        const [deployer, user1, user2, topup] = await ethers.getSigners()

        const lpToken = new ethers.Contract(addresses.LPTOKENS.mGOOGL_UST, IERC20_ABI, deployer)
        ...
        await lpToken.connect(unlockedUser).transfer(user1.address, ethers.utils.parseUnits("2", "18"))
        await lpToken.connect(unlockedUser2).transfer(user2.address, ethers.utils.parseUnits("0.7", "18"))
    ...
    beforeEach(async () => {
        await deployments.fixture(["mirror_mainnet_deploy_pool_mgoogl_ust"])
    })
