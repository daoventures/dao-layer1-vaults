...
const unlockedAddress = "0x7ba605bc00ea26512a639d5e0335eaeb3e81ad94"//"0xc247722Ac42B2f9BA752886502c3D3dD39BDb2Da"//addresses.ADDRESSES.unlockedUser
const unlockedAddress2 = "0x4ec7cdf61405758f5ced5e454c0b4b0f4f043df0" //"0xF39d30Fa570db7940e5b3A3e42694665A1449E4B"//addresses.ADDRESSES.unlockedUser2


describe("Mirror - mTWTR-UST", () => {
    const setup = async () => {
        const [deployer, user1, user2, topup] = await ethers.getSigners()

        const lpToken = new ethers.Contract(addresses.LPTOKENS.mTWTR_UST, IERC20_ABI, deployer)
        ...
        await lpToken.connect(unlockedUser).transfer(user1.address, ethers.utils.parseUnits("1", "18"))
        await lpToken.connect(unlockedUser2).transfer(user2.address, ethers.utils.parseUnits("0.0009", "18"))
    ...
    beforeEach(async () => {
        await deployments.fixture(["mirror_mainnet_deploy_pool_mtwtr_ust"])
    })
