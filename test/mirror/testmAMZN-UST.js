...
const unlockedAddress = "0xd91200d7652b93ec38758e94618cdc3eb10a19f5"//"0xc247722Ac42B2f9BA752886502c3D3dD39BDb2Da"//addresses.ADDRESSES.unlockedUser
const unlockedAddress2 = "0x7a48f0fc1dbe237e0073a2dc745e4c9f756509cd" //"0xF39d30Fa570db7940e5b3A3e42694665A1449E4B"//addresses.ADDRESSES.unlockedUser2


describe("Mirror - mAMZN-UST", () => {
    const setup = async () => {
        const [deployer, user1, user2, topup] = await ethers.getSigners()

        const lpToken = new ethers.Contract(addresses.LPTOKENS.mAMZN_UST, IERC20_ABI, deployer)
        ...
        await lpToken.connect(unlockedUser).transfer(user1.address, ethers.utils.parseUnits("1.2", "18"))
        await lpToken.connect(unlockedUser2).transfer(user2.address, ethers.utils.parseUnits("0.04", "18"))
    ...
    beforeEach(async () => {
        await deployments.fixture(["mirror_mainnet_deploy_pool_mamzn_ust"])
    })
